<!-- section "Fruit, légume" renvoie sur le catalogue -->
<section class="my-6 px-3 px-lg-0" data-aos="fade-up">
    <div class="container bg-warning rounded-5 p-0">
        <div class="row g-0 align-items-center">
            <div class="col-xl-6">
                <div class="p-5 p-lg-6">
                    <h2 class="title-lilita fs-1 mb-4">Fruits, légumes, fromages...</h2>
                    <h5 class="mb-4">Choisissez parmi une vaste sélection de produits et effectuez toutes vos courses en direct, provenant de petites fermes situées à moins de 30 km.</h5>
                    <div>
                        <a class="btn btn-success icon-link icon-link-hover my-1" href="/controllers/catalog-ctrl.php">
                            Commencer mon marché
                            <i class="bi bi-chevron-right mb-2"></i>
                        </a>
                    </div>
                </div>
            </div>
            <div class="col-xl-6">
                <img src="/public/assets/img/legumes-hiver.webp" class="img-baner-shop object-fit-cover" alt="photo Localco de légumes d'hiver">
            </div>
        </div>
    </div>
</section><!-- FIN section "Fruit, légume" -->